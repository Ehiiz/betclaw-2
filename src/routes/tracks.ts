  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        logger.error({ err, trackId: track._id }, 'Failed to start track loop')
      }
    })
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tracks = await BetTrack.find({ userId: req.user!.userId }).sort({ createdAt: -1 })
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const track = await BetTrack.findOne({ _id: req.params.id, userId: req.user!.userId })
...
router.get('/:id/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const progressToTarget = ((track.budget + track.totalPnL) / track.target) * 100
...
router.get('/:id/history', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        track: { _id: track._id, name: track.name, totalPnL: track.totalPnL, sessionCount: track.sessionCount },
        sessions,
      },
    })
  } catch (err) {
...
router.patch('/:id/pause', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const track = await BetTrack.findOne({ _id: req.params.id, userId: req.user!.userId })
    if (!track) throw new AppError(404, 'Track not found')
    if (track.status !== TrackStatus.ACTIVE) throw new AppError(400, 'Only active tracks can be paused')
...
router.patch('/:id/resume', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const track = await BetTrack.findOne({ _id: req.params.id, userId: req.user!.userId })
    if (!track) throw new AppError(404, 'Track not found')
    if (track.status !== TrackStatus.PAUSED) throw new AppError(400, 'Only paused tracks can be resumed')
    ...
    setImmediate(async () => {
      try {
        ...
        const { logger } = await import('../config/logger')
        logger.error({ err, trackId: track._id }, 'Failed to restart track loop on resume')
      }
    })
